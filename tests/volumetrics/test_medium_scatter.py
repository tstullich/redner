...
pyredner.imwrite(target.cpu(), 'results/test_medium_scatter/target.exr')
pyredner.imwrite(target.cpu(), 'results/test_medium_scatter/target.png')
target = pyredner.imread('results/test_medium_scatter/target.exr')
if pyredner.get_use_gpu():
    target = target.cuda(device = pyredner.get_device())

# Before perturbing save old value
target_val = torch.tensor(mediums[0].sigma_s, device=pyredner.get_device())
...
## Save image
pyredner.imwrite(img.cpu(), 'results/test_medium_scatter/init.png')
...
with open('results/test_medium_scatter/cornell-scatter-param.csv', 'w') as file:
    file.write('a b')
    file.write('\n')
...
for t in range(100):
    ...
    pyredner.imwrite(img.cpu(), 'results/test_medium_scatter/iter_{}.png'.format(t))
    ...
    with open('results/test_medium_scatter/cornell-scatter-param.csv', 'a') as file:
        file.write(str(t) + ' ' + str(torch.abs(target_val - mediums[0].sigma_s).sum().item()))
        file.write('\n')

    # Backpropagate the gradients
    loss.backward()
    # Print the gradients of the absorption factor
    print('grad:', mediums[0].sigma_s.grad)

    # Take a gradient descent step
    optimizer.step()
    # Clamp sigma_s to a valid value
