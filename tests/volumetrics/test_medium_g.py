...
pyredner.imwrite(target.cpu(), 'results/test_medium_g/target.exr')
pyredner.imwrite(target.cpu(), 'results/test_medium_g/target.png')
target = pyredner.imread('results/test_medium_g/target.exr')
if pyredner.get_use_gpu():
    target = target.cuda(device = pyredner.get_device())

# Before perturbing save old value
target_val = torch.tensor(mediums[0].g, device=pyredner.get_device())
...
## Save image
pyredner.imwrite(img.cpu(), 'results/test_medium_g/init.png')
...
# Setup loss csv file
with open('results/test_medium_g/cornell-g-loss.csv', 'w') as file:
    file.write('a b')
    file.write('\n')

# Setup param diff file
with open('results/test_medium_g/cornell-g-param.csv', 'w') as file:
    file.write('a b')
    file.write('\n')

# Optimize absorption factor of medium inside the sphere
optimizer = torch.optim.Adam([mediums[0].g], lr=5e-3)
...
for t in range(100):
    ...
    pyredner.imwrite(img.cpu(), 'results/test_medium_g/iter_{}.png'.format(t))

    # Compute the loss function
    loss = (img - target).pow(2).sum()
    print('loss:', loss.item())

    with open('results/test_medium_g/cornell-g-loss.csv', 'a') as file:
        file.write(str(t) + ' ' + str(loss.item()))
        file.write('\n')

    with open('results/test_medium_g/cornell-g-param.csv', 'a') as file:
        file.write(str(t) + ' ' + str(torch.abs(target_val - mediums[0].g).sum().item()))
        file.write('\n')
...
with open('results/test_medium_g/final_val.txt', 'w') as file:
...
pyredner.imwrite(img.cpu(), 'results/test_medium_g/final.exr')
pyredner.imwrite(img.cpu(), 'results/test_medium_g/final.png')
pyredner.imwrite(torch.abs(target - img).cpu(), 'results/test_medium_g/final_diff.png')